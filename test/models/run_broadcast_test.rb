...
class RunBroadcastTest < ActiveSupport::TestCase
  test "control room broadcast replaces the mounted run panes" do
    ...
    assert_equal 6, streams.size
    assert_equal %w[session_sidebar run_header passport_tree permission_inbox audit_timeline passport_detail], streams.map { |stream| stream["target"] }
