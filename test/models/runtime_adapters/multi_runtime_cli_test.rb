...
class RuntimeAdapters::MultiRuntimeCliTest < ActiveSupport::TestCase
  ...
  test "starts Pi with JSON mode and read-only tools" do
    run = create_run(runtime_name: "pi")
    process = FakeProcess.new
    cli = RuntimeAdapters::PiCli.new(command: "pi-test", process: process, check_available: false)
    ...
    assert_equal "pi", env.fetch("AGENT_PASSPORTS_RUNTIME_NAME")
    assert_equal run.id.to_s, env.fetch("AGENT_PASSPORTS_RUN_ID")
    assert_equal run.bridge_token, env.fetch("AGENT_PASSPORTS_BRIDGE_TOKEN")
    assert_equal "pi-test", process.spawn_args[1]
    assert_equal "json", process.spawn_args[process.spawn_args.index("--mode") + 1]
    assert_includes process.spawn_args, "--no-session"
    assert_includes process.spawn_args, "--no-approve"
    assert_equal "read,grep,find,ls", process.spawn_args[process.spawn_args.index("--tools") + 1]
    assert_equal "Agent Identity Control Room demo", process.spawn_args[process.spawn_args.index("--name") + 1]
    assert_equal run.project_path, options.fetch(:chdir)
    assert_match %r{log/pi-demo-run-#{run.id}\.log\z}, options.fetch(:out)
    assert_equal [ :child, :out ], options.fetch(:err)
  end
end
