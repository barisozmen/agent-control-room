class RuntimeObserverEventsController < ApplicationController
  ...
  def require_json_bridge_request!
    ...
    raise UnsupportedBridgeMediaType, "Runtime observer events must be posted as JSON"
  end

  def runtime_event_params
    payload = params[:runtime_event].presence || params[:opencode_event].presence || params
    payload.to_unsafe_h.except("controller", "action", "runtime_name").with_indifferent_access
  end

  def runtime_event_response(ingestion)
