...
module RuntimeAdapters
  class PiProcessScanner
    Session = Data.define(:pid, :started_at, :command, :cwd) do
      ...
      def session_id
        "pi-process-#{pid}-#{started_at.to_i}"
      end
    end
    ...
    def sessions
      ps_output
        ...
        .select { |process| pi_cli_session?(process[:command]) }
        .first(limit)
    ...
    def pi_cli_session?(command)
      return false unless command.match?(%r{(^|\s|/)pi(\s|$)})
      return false if command.match?(/\bpi\s+(install|remove|uninstall|update|list|config)\b/)
      return false if command.match?(/\bpi\s+--(?:help|version|list-models|export)\b/)
      return false if command.match?(/\bpi\s+-(?:h|v)\b/)
    ...
    def capture(*command)
      ...
      Rails.logger.debug("Pi process scan skipped #{command.first}: #{error.class}: #{error.message}")
      ""
    end
  end
end
