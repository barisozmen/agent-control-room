...
module ObservedOpencodeSessions
  class Ingestor
    ...
    def find_or_create_run!
      Run.find_or_initialize_by(runtime_name: "opencode", runtime_session_id: session_id).tap do |record|
    ...
    def ensure_base_passports!
      ...
      run.passports.find_or_create_by!(actor_ref: MAIN_AGENT_REF) do |passport|
        assign_passport(passport, actor_name: "opencode/main-agent", actor_kind: "agent", provider: "opencode", parent: owner, task: "Observed OpenCode session", rules: AGENT_RULES)
      end
    end
    ...
    def ensure_actor_passport!
      ...
      run.passports.create! do |passport|
        assign_passport(
          ...
          provider: event[:provider].presence || "opencode",
          parent: parent,
          task: event[:task].presence || "Observed OpenCode actor",
    ...
    def session_id
      @session_id ||= event[:session_id].presence ||
        ...
        "opencode-process-#{observed_pid || Digest::SHA256.hexdigest("#{project_path}:#{title}")[0, 16]}"
    end
