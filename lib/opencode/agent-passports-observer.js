  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      try {
        ...
        if (immediateDecision === "deny") throw new Error("Agent Control Room denied this tool call")
        if (response?.status === "asking") {
          const decision = await waitForPermissionDecision(response)
          if (decision === "deny") throw new Error("Agent Control Room denied this tool call")
        }
      } catch (error) {
...
async function postOpencodeEvent(payload) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Agent Control Room observer bridge failed: ${response.status} ${body}`)
  }
...
function explicitDenial(error) {
  return String(error?.message || "").includes("Agent Control Room denied")
}

async function logBridgeError(hook, error) {
  if (process.env.AGENT_PASSPORTS_DEBUG) {
    console.error(`Agent Control Room observer ${hook} failed open:`, error)
  }
}
